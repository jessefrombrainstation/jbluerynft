...
class MoralisWeb3 {
    ...
    for (let i = 0; i < triggersArray.length; i++) {
      switch (triggersArray[i].name) {
        ...
        case 'web3Sign':
          ...
          if (!triggersArray[i].signer || triggersArray[i].signer) throw new Error('web3Sign trigger signer address missing or invalid'); // eslint-disable-next-line no-case-declarations

          let response = await this.getInternalWeb3Provider().signer.signMessage(triggersArray[i].message);
  ...
  static async executeFunction({
    ...
    const response = await contract[functionName](...Object.values(parsedInputs), msgValue ? {
